...
const MemberManage = () => {
  ...
  const editMember = async (e, member) => {
    ...
    try {
      dispatch(changeLoadingState(true));
      await axios.put(`${baseUrl}/api/${apiPath}/admin/article/${member.id}`, updateData, config);
      dispatch(
        pushMsg({
          text: '已修改會員狀態',
          status: 'success',
  ...
  return (
    <>
      <div className="container py-10 memberManage">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h1 className="h4 my-4">會員管理</h1>
        </div>
        {memberInfo.length === 0 ? (
          <div className="d-flex justify-content-center align-items-center pt-10 text-grey-400 memberWrap">
            <h2 className="h5">目前沒有會員資料</h2>
          </div>
        ) : (
              ...
              {memberInfo.map((member) => (
                <tr key={member.id}>
                  ...
                  <td className="text-center py-5">
                    ...
                    {/* 手機版 */}
                    <button
                      onClick={(e) => editMember(e, member)}
                      name="isAdmin"
                      value="0"
                      style={{ whiteSpace: `nowrap` }}
                      type="button"
                      className={`btn text-white d-lg-none ${member.isAdmin == 0 ? 'd-none' : ' btn-brand-800'}`}
                    >
                      管理員
                    </button>
                    ...
                    <button
                      ...
                      style={{ whiteSpace: `nowrap` }}
                      type="button"
                      className={`btn text-white d-lg-none ${member.isAdmin == 1 ? 'd-none' : ' btn-brand-400'}`}
                    >
                      會員
                    </button>
                  </td>
                  ...
                  <td className="text-center py-5">
                    ...
                    <select
                      className="form-select d-none d-lg-block"
                      name="isPublic"
                      value={member.isPublic ? 'true' : 'false'}
                      onChange={(e) => editMember(e, member)}
                    >
                    ...
                    <button
                      ...
                      style={{ whiteSpace: `nowrap` }}
                      type="button"
                      className={`btn text-white d-lg-none ${member.isPublic == 0 ? 'd-none' : ' btn-brand-400'}`}
                    >
                      正常
                    </button>
                    ...
                    <button
                      ...
                      style={{ whiteSpace: `nowrap` }}
                      type="button"
                      className={`btn text-white d-lg-none ${member.isPublic == 1 ? 'd-none' : ' btn-brand-800'}`}
                    >
                      封鎖
                    </button>
                  </td>
