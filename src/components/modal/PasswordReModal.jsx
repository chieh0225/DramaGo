...
const PasswordReModal = ({ mymodal, mypasswordRef }) => {
  ...
  //表單
  const onSubmit = () => {
    if (watch('numNew') == watch('numOld')) alert(`不可與舊密碼相同`);
    else {
      login();
    }
  };
          ...
          <div className="row">
            ...
            <div className="col-lg-6 py-8 px-6 ps-lg-3">
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                <div className="mb-3">
                  <label htmlFor={`PasswordRe${id}`} className="form-label">
                    請輸入舊密碼
                  </label>
                  <input
                    {...register('numOld', {
                ...
                <div className="mb-3">
                  <label htmlFor={`PasswordNew${id}`} className="form-label">
                    請輸入新密碼
                  </label>
                  <input
                    {...register('numNew', {
              ...
              <button className="btn w-100 rounded-pill btn-brand-400 mb-4" onClick={login}>
                返回登入
              </button>
            </div>
          </div>
